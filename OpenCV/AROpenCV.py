...
imgTarget = cv2.imread("download.png")
...
while True:
    ...
    for m, n in matches:
        if m.distance < 0.75 * n.distance:
            good.append(m) print(len(good))     imgFeatures = cv2.drawMatches(imgTarget, kp1, imgWebcam, kp2, good, None, flags=2) if len(good) > 20:
        detection = True
        srcPts = np.float32(
        ...
        img2 = cv2.polylines(
            imgWebcam, [np.int32(dst)], True, (255, 0, 255), 3)
